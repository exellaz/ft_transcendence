...
export function closeMatchWebsocket(roomId: number, playerId: number) {
    ...
    if (ws) {
        try { ws.close(1000, "match room disconnected"); } catch (e) {}
...
export function useMatchWebsocket(
    ...
    const normalizeToMatchPlayer = (raw: unknown): MatchPlayer => {
        ...
        const username = typeof obj.playerName === "string" ? obj.playerName : typeof obj.username === "string" ? obj.username : "";
        const spriteUrl = typeof obj.spriteUrl === "string" ? obj.spriteUrl : (typeof obj.sprite === "string" ? obj.sprite : "");
    ...
    useEffect(() => {
        if (!roomId || roomId <= 0 || !player.id || player.id <= 0) {
            console.error("[match-websocket] invalid roomId or playerId, cannot establish websocket connection: ", roomId, player.id); ////debug
            return;
        }
        ...
        ws.onopen = () => console.log("[match-websocket] WebSocket connection opened");

        ws.onerror = (err) => console.error("[match-websocket] WebSocket error:", err);
        ...
        ws.onmessage = (event) => {
		    ...
		    if (data && data.type === "handshakePing") {
                ...
                const leftPlayer = Array.isArray(data.PlayerInfo.teams.left) ? data.PlayerInfo.teams.left : [];
                const rightPlayer = Array.isArray(data.PlayerInfo.teams.right) ? data.PlayerInfo.teams.right : [];
                const left = leftPlayer.map((p: unknown) => normalizeToMatchPlayer(p));
                const right = rightPlayer.map((p: unknown) => normalizeToMatchPlayer(p));
            ...
            if (data.type === "roleUpdate") {
                console.log("[match-websocket] roleUpdate data:", data.gameState.teams.left[0].ready, data.gameState.teams.right[0].ready); ////debug
                const leftPlayer = Array.isArray(data.gameState.teams.left) ? data.gameState.teams.left : [];
                const rightPlayer = Array.isArray(data.gameState.teams.right) ? data.gameState.teams.right : [];
                const left = leftPlayer.map((p: unknown) => normalizeToMatchPlayer(p));
                const right = rightPlayer.map((p: unknown) => normalizeToMatchPlayer(p));
        ...
        ws.onclose = (ev) => {
            console.log("[match-websocket] WebSocket connection closed for room", roomId, {
    ...
    function handleRoomReady(ready:boolean) {
        if (socketRef.current && socketRef.current.readyState === WebSocket.OPEN) {
            const message = JSON.stringify({ type: "ready", ready, matchGameStart: true });
