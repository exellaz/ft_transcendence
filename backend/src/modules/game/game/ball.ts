...
export class Ball extends GameObject {
	...
	calculateAngle(collidingPadel: Padel) {
		// get center of padel
		const paddleCenterY = collidingPadel.position.y + collidingPadel.scale.y / 2;
		...
		const normalizedIntersectY = relativeIntersectY / (collidingPadel.scale.y / 2);
	...
	start(team: Team = Team.TEAM_LEFT) {
		this.game.ball.velocity.x = this.game.gameSettings.ballSpeed * (team === Team.TEAM_LEFT ? 1: -1);
	}
