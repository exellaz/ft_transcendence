	...
	init() {
		...
		this.addChild(new ImageObject({
			...
			interpolate: new Interpolate({
				targetOffset: new Vector2D(this.team === Team.TEAM_RIGHT ? -3 : 3, -3),
				target: this,
				slowness: 1.7,
			})
		}));
		...
		this.addChild(new ImageObject({
			...
			interpolate: new Interpolate({
				targetOffset: new Vector2D(this.team === Team.TEAM_RIGHT ? -6 : 6, -3),
				target: this,
				slowness: 2.5,
			}),
		}));
...
export class Arrow extends GameObject {
	...
	constructor(params: Partial<Arrow>) {
		...
		this.addChild(new Label({
			text: this.text,
			game: this.game,
			hAlign: (this.team === Team.TEAM_LEFT) ? HorizontalAlign.Left : HorizontalAlign.Right,
			font: "bold 15px Century Gothic",
			color: "#ffffff",
			outline: new Outline({}),
			// rotation: 90,
	...
	draw(viewport: Viewport){
		...
		else if (this.team === Team.TEAM_RIGHT && screenPosition.x > viewport.width) {
			super.draw(viewport);
		}
	}
